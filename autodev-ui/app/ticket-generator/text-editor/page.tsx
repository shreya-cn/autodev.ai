      ...
      <div className="space-y-8">
        ...
        <div className="rounded-2xl px-7 py-7 text-white border border-green-500/20 shadow-xl" style={{background: 'linear-gradient(135deg, #1a1a1a 0%, #2d4a2e 50%, #1a1a1a 100%)'}}>
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-2 leading-tight">
            AI Ticket Generator - <span style={{color: '#b9ff66'}}>Text Editor</span>
          </h1>
          <p className="text-sm md:text-base lg:text-lg text-gray-300 leading-relaxed max-w-4xl">
            Describe your problem or enhancement, and let AI create a comprehensive ticket
          </p>
        </div>

        {/* Input Section */}
        <div className="bg-dark rounded-lg shadow-md p-6 border border-green-500/20">
          <label htmlFor="userInput" className="block text-lg font-semibold text-white mb-3">
          ...
          <p className="text-sm text-gray-400 mt-2">
            Be as detailed as possible. Include context about the issue, expected behavior, and any relevant technical details.
          </p>
          
          <button
            onClick={handleGenerate}
            disabled={isGenerating || !userInput.trim()}
            style={{backgroundColor: isGenerating || !userInput.trim() ? '' : '#b9ff66'}}
        ...
        {generatedTicket && (
          <div className="bg-dark rounded-lg shadow-md p-6 space-y-6 border border-green-500/20">
            <div className="flex items-center justify-between border-b border-gray-700 pb-4">
              <h2 className="text-2xl font-bold text-white">Generated Ticket</h2>
              <div className="flex gap-2">
                ...
                <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                  generatedTicket.suggestedType === 'Bug' ? 'bg-red-900/40 text-red-300 border border-red-500/50' :
                  generatedTicket.suggestedType === 'Story' ? 'bg-blue-900/40 text-blue-300 border border-blue-500/50' :
                  generatedTicket.suggestedType === 'Epic' ? 'bg-purple-900/40 text-purple-300 border border-purple-500/50' :
                  'bg-gray-800 text-gray-300 border border-gray-600'
                }`}>
                  {generatedTicket.suggestedType}
                </span>
                <span className={`px-3 py-1 rounded-full text-sm font-semibold ${
                  generatedTicket.suggestedPriority === 'Highest' || generatedTicket.suggestedPriority === 'High' 
            ...
            <div>
              ...
              <p className="text-gray-300 bg-gray-800 p-3 rounded border border-gray-700">{generatedTicket.summary}</p>
            </div>
            ...
            {generatedTicket.acceptanceCriteria.length > 0 && (
              <div>
                ...
                <ul className="list-disc list-inside space-y-2 bg-gray-800 p-4 rounded border border-gray-700">
                  {generatedTicket.acceptanceCriteria.map((criterion, index) => (
                    <li key={index} className="text-gray-300">{criterion}</li>
                  ))}
                </ul>
              </div>
            )}
            ...
            {generatedTicket.technicalNotes && (
              <div>
                <h3 className="text-lg font-semibold text-white mb-2">Technical Notes</h3>
                <div className="text-gray-300 bg-gray-800 p-4 rounded whitespace-pre-wrap border border-gray-700">
                  {generatedTicket.technicalNotes}
                </div>
              </div>
            ...
            {generatedTicket.relatedComponents.length > 0 && (
              <div>
                <h3 className="text-lg font-semibold text-white mb-2">Related Components</h3>
                <div className="flex flex-wrap gap-2">
                  {generatedTicket.relatedComponents.map((component, index) => (
                    <span key={index} className="px-3 py-1 bg-purple-900/40 text-purple-300 border border-purple-500/50 rounded-full text-sm">
                      {component}
                    </span>
                  ))}
                </div>
              ...
              <div className="bg-yellow-900/20 border border-yellow-500/50 rounded-lg p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-yellow-400 mb-2">Large Story Detected ({generatedTicket.storyPoints} Story Points)</h3>
                    {generatedTicket.suggestedType === 'Epic' ? (
                      <div>
                        <p className="text-red-300 mb-4 font-semibold">
                          ⚠️ Warning: This was generated as an Epic, but Epics cannot have subtasks in Jira. 
                          Please regenerate as a Story type if you want to break it into subtasks.
                        </p>
                        <button
                          onClick={() => {
                            setGeneratedTicket(null);
                      ...
                      <div>
                        <p className="text-gray-300 mb-4">
                          This ticket has more than 8 story points. Would you like to break it down into smaller subtasks for better tracking and implementation?
                        </p>
                        {generatedTicket.subtasks && generatedTicket.subtasks.length > 0 && (
                          <div className="mb-4">
                            <p className="text-sm font-semibold text-white mb-2">Suggested Subtasks ({generatedTicket.subtasks.length}):</p>
                            <ul className="space-y-2">
                              {generatedTicket.subtasks.map((subtask, index) => (
                                <li key={index} className="bg-gray-800 p-3 rounded border border-gray-700">
                                  <div className="flex items-start justify-between">
                                    <span className="text-gray-300 flex-1">{subtask.summary}</span>
                                    <span className="text-xs px-2 py-1 bg-gray-700 rounded text-gray-400 ml-2">{subtask.storyPoints} SP</span>
                                  </div>
                                </li>
                              ))}
                            </ul>
            ...
            <div className="pt-4 border-t border-gray-700 flex gap-4">
              ...
              >
                {isCreating ? 'Creating Ticket...' : createSubtasks && generatedTicket.subtasks && generatedTicket.subtasks.length > 0 

...
class JavaDocumentationMCPServer {
    ...
    logError(message, data) {
        ...
        if (data instanceof Error) {
            ...
            this.log('error', message, typeof data === 'object' ? JSON.parse(JSON.stringify(data)) : String(data));
        }
        else {
            this.log('error', message);
        }
  ...
  inputSchema: {
    ...
    async getLastCommitFromBranch(projectPath, branch) {
    ...
    async fetchJiraIssue(ticketNumber) {
    ...
    async calculateSourceHash(projectPath) {
        const javaFiles = await this.findJavaFiles(projectPath);
    ...
    async shouldRegenerateClassesSummary(projectPath, forceRegenerate = false) {
        if (forceRegenerate) {
    ...
    async saveCacheMetadata(projectPath, classesSummary) {
    ...
    async shouldRegenerateDocumentation(forceRegenerate = false, classesSummaryWasRegenerated = false) {
        if (forceRegenerate) {
    ...
    async parseJavaFiles(projectPath) {
    ...
    parseJavaClass(content) {
        ...
        if (annotationMatches) {
            annotationMatches.forEach(annotation => {
                ...
                if (cleanAnnotation === 'Entity')
                    isEntity = true;
                if (cleanAnnotation === 'RestController')
    ...
    async reviewCodeChanges(args: { diff: string; files?: string[] }) {
...
This PR may include any programming language.
`;

  const userPrompt = `
Changed files:
${args.files?.length ? args.files.join('\n') : 'Not provided'}
...
`;
  ...
  return {
    ...
    async generateArchitectureDesign(classesSummary, outputFormat) {
...
Make it suitable for architects and senior developers to understand the overall system design.`;
    ...
    async generateDetailedDesign(classesSummary, outputFormat) {
...
Make it suitable for developers to understand the detailed implementation design.`;
    ...
    async generateOpenApiSpecification(classesSummary, outputFormat) {
        ...
        const mainAppClass = classesSummary.find(cls => cls.annotations?.includes("SpringBootApplication"));
        if (!mainAppClass) {

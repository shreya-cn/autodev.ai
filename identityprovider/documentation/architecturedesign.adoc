...
Authentication is handled via `UserController` which uses `JWTService` to create and validate tokens. `SecurityConfig` configures HTTP security, applying `JwtAuthFilter` to secure endpoints.